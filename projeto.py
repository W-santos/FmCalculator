        ...
        for row in range(self.sheet.nrows):
            ...
            try:
                ...
                if dist < 300 and channel in ch:
                    self.processinter(v+[dist])

    def processinter(self,erb):
        ...
        if cib > 0:
            ...
            d1 = 'Distancia protegida para o item {} ({}) = {:.3} e distancia interferente = {:.3}\n'.format(erb[0],
                                                                                                       erb[14],
                                                                                                       distp1,
                                                                                                       distint1)
            memo.write(d1)
            memo.write('A interferindo em B\n')
            d1 = 'Distancia protegida para a base  = {:.3} e distancia interferente = {:.3}\n'.format(distp2, distint2)
            memo.write(d1)
            ...
            d1 = calculo.tvfmfs_metric(pot,haat,FM,85,0,2,1,[])
            d2 = calculo.tvfmfs_metric(poterp[self.classe],haaterp[self.classe],FM,85,0,2,1,[])
            print(d1,d2)
            memo.write('Contorno protedido de E(50,50) para 85 dBu para o item {} ({}) = {:.3}\n'.format(erb[0],erb[14],d1))
            memo.write('Contorno protedido de E(50,50) para 85 dBu para a base = {:.3}\n'.format(d2))
            if d1 + d2 < erb[-1]:
                memo.write('Não há problema')
            else:
                memo.write('Interferência')
